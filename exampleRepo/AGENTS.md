...
- Start with this file for repository-wide context.
- Read `AGENTS.atlas.md` when choosing an existing Context Map or Context Route.
- Read child `AGENTS.md` files only for the subtree being changed.
- Prefer a Context Map when the task needs cross-subtree references.
- Prefer a Context Route when a repeated workflow depends on reading context in a specific order.
