...
Rules: Core API runtime logic.
- Preserve static-file serving path safety checks and JSON error handling behavior.
- Keep route handlers pure to request scope except for explicit persistence calls via the store.
Validate: `node --check apps/api/src/server.js && node --check apps/api/src/http.js`
