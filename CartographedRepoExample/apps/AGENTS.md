...
Rules: application-layer constraints.

- Keep server-side HTTP handling in `/apps/api` and browser runtime behavior in `/apps/web`.
- Keep cross-app reusable utilities in `/packages` instead of duplicating implementation across app siblings.
- When a change touches API contracts consumed by the web app, update both subtrees in one scoped change set.
