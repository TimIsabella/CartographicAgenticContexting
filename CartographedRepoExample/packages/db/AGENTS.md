...
Rules: JSON-backed persistence store.
- Preserve database shape normalization in `ensureDbShape`.
- Keep writes atomic at file granularity through `writeDb` and `updateDb`.
Validate: `node --check packages/db/store.js`
