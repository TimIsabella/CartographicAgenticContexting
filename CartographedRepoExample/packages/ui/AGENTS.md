...
Rules: shared UI package constraints.

- Keep component factories focused on DOM creation without network or API side effects.
- Export only reusable primitives that can be consumed by browser-facing code in `apps/web`.
- Keep styling assumptions minimal so host apps can control layout and theming.
