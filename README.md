...
| Map | `AGENTS.map.<name>.md` | Pointers for one area, workflow, or concern | When the task matches the map name or purpose |
| Route | `AGENTS.route.<name>.md` | Ordered context traversal for a task | When task setup depends on reading context in sequence |
```
